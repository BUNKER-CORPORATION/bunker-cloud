...
// Multipart for file uploads (100MB limit for direct uploads)
...
await fastify.register(bucketRoutes);
...
const start = async () => {
  try {
    ...
    console.log(`Storage service running on ${config.host}:${config.port}`);
  } catch (err) {
